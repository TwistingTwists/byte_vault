    ...
    <div className="w-full mt-2 mb-6 px-6 py-4 bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-md border border-gray-700">
      ...
      >
        ...
        {events.map((event, index) => {
          ...
          let labelBgColor = 'bg-gray-800/20';
          let labelBorderColor = 'border-gray-700/30';
          
          if (event.tx === 'T1') {
            bgColor = isActive ? 'bg-cyan-500' : 'bg-cyan-800/50';
            glowColor = 'shadow-[0_0_8px_0_rgba(34,211,238,0.4)]';
            if (isCurrent) {
              labelBgColor = 'bg-cyan-900/50';
              labelBorderColor = 'border-cyan-500/40';
            }
            if (event.highlight) {
              bgColor = isActive ? 'bg-yellow-500' : 'bg-yellow-800/50';
              glowColor = 'shadow-[0_0_8px_0_rgba(234,179,8,0.4)]';
              if (isCurrent) {
                labelBgColor = 'bg-yellow-900/50';
                labelBorderColor = 'border-yellow-500/40';
              }
            }
          } else if (event.tx === 'T2') {
            ...
            if (isCurrent) {
              labelBgColor = 'bg-orange-900/50';
              labelBorderColor = 'border-orange-500/40';
            }
            if (event.highlight) {
              bgColor = isActive ? 'bg-yellow-500' : 'bg-yellow-800/50';
              glowColor = 'shadow-[0_0_8px_0_rgba(234,179,8,0.4)]';
            ...
            if (isCurrent) {
              labelBgColor = 'bg-indigo-900/50';
              labelBorderColor = 'border-indigo-500/40';
            }
          }
          ...
          return (
            <div key={index} 
              ...
              {/* Event label - with staggered positioning and highlighted active step */}
              <div 
                className={`absolute max-w-40 whitespace-nowrap text-xs font-medium ${textColor} px-2.5 py-1.5 rounded-lg 
                  ${labelPosition}
                  ${isCurrent ? labelBgColor : bgColor.replace('bg-', 'bg-')}/10 backdrop-blur-sm 
                  border ${isCurrent ? labelBorderColor : 'border-gray-700/30'} shadow-lg 
                  ${isCurrent ? 'font-bold scale-105 z-10' : ''} 
                  transition-all duration-300`}
                style={{ 
                  transform: `translateY(${yOffset}px)`,
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  boxShadow: isCurrent ? '0 0 12px rgba(255,255,255,0.15)' : '',
                  zIndex: isCurrent ? 50 : 'auto'  // This ensures current step is always on top
                }}
              >
                {event.action}
              </div>
