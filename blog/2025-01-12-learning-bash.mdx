---
slug: "bash-from-basics-01"
title: "Bash from Basics - 01"
...
The advantage of pushd and popd is that they manage a stack of directories, making it easy to switch between multiple locations and return to previous ones without having to manually remember and type the paths. With cd alone, you'd have to store the original directory in a variable and then cd back to it explicitly. For simple cases, cd is fine, but pushd and popd are more convenient for complex scenarios involving multiple directory changes.