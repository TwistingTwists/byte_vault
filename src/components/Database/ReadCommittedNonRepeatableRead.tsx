...
interface ScenarioKeyMomentHighlight {
  ...
  nonRepeatableRead?: { txName: string; item: string }; // Special highlight for non-repeatable read
}
...
const nonRepeatableReadScenario: ReadCommittedScenarioConfig = {
  name: "Read Committed: Non-repeatable Read Anomaly",
  description: "Demonstrates how Read Committed isolation allows the same query within a transaction to return different results when other transactions commit changes between the reads.",
  initialDataItems: {
    AccountA: [{ id: 'vA0', value: 500, txMin: 0, txMax: null, txMinName: 'Initial', txMaxName: null }],
    AccountB: [{ id: 'vB0', value: 300, txMin: 0, txMax: null, txMinName: 'Initial', txMaxName: null }]
  },
  transactions: {
    T1: {
      color: '#3b82f6', // Blue-500
      operations: [
        { type: 'begin', time: 0, comment: "Transfer $100 from A to B" },
        { type: 'read', time: 5, target: 'AccountA' },
        { type: 'write', time: 10, target: 'AccountA', value: 400, comment: "500 - 100" },
        { type: 'read', time: 15, target: 'AccountB' },
        { type: 'write', time: 20, target: 'AccountB', value: 400, comment: "300 + 100" },
        { type: 'commit', time: 25 }
      ]
    },
    T2: {
      ...
      operations: [
        { type: 'begin', time: 8, comment: "Check total balance" },
        { type: 'read', time: 12, target: 'AccountA', comment: "First read" },
        { type: 'read', time: 18, target: 'AccountB', comment: "First read" },
        { type: 'read', time: 28, target: 'AccountA', comment: "Second read - same query!" },
        { type: 'read', time: 30, target: 'AccountB', comment: "Second read - same query!" },
        { type: 'commit', time: 35 }
      ]
    }
  },
  keyMoments: [
    { step: 3, text: "T2 reads AccountA = 500. T1's write is uncommitted, so T2 sees the initial value.", autoPause: true, highlight: { currentOpPanel: true, transactionSpecificRead: { txName: 'T2', item: 'AccountA', versionId: 'vA0', step: 3 } } },
    { step: 5, text: "T2 reads AccountB = 300. T1's write is uncommitted, so T2 sees the initial value. Total: 500 + 300 = 800", autoPause: true, highlight: { currentOpPanel: true, transactionSpecificRead: { txName: 'T2', item: 'AccountB', versionId: 'vB0', step: 5 } } },
    { step: 6, text: "T1 commits the transfer. AccountA = 400, AccountB = 400 are now visible to new reads.", autoPause: true, highlight: { currentOpPanel: true, committedTx: 1, dataItemVersions: { AccountA: ['vA1'], AccountB: ['vB1'] } } },
    { step: 7, text: "NON-REPEATABLE READ! T2 reads AccountA again and gets 400 (not 500). Same query, different result because T1 committed!", autoPause: true, isCritical: true, highlight: { currentOpPanel: true, transactionSpecificRead: { txName: 'T2', item: 'AccountA', versionId: 'vA1', step: 7 }, nonRepeatableRead: { txName: 'T2', item: 'AccountA' } } },
    { step: 8, text: "NON-REPEATABLE READ! T2 reads AccountB again and gets 400 (not 300). The total is still 800, but individual values changed during T2's execution!", autoPause: true, isCritical: true, highlight: { currentOpPanel: true, transactionSpecificRead: { txName: 'T2', item: 'AccountB', versionId: 'vB1', step: 8 }, nonRepeatableRead: { txName: 'T2', item: 'AccountB' } } }
  ]
};


// --- Read Committed Non-repeatable Read Scenario ---
let versionCounter = 0;
const generateVersionId = (prefix: string): string => `${prefix}${versionCounter++}`;


const ReadCommittedNonRepeatableReadVisualizer: React.FC = () => {
  ...
  const { transactions, keyMoments, initialDataItems: scenarioInitialDataItems } = nonRepeatableReadScenario;
  ...
  const initializeState = useCallback(() => {
    ...
    Object.keys(initialClonedData).forEach(itemName => {
        initialClonedData[itemName].forEach(version => {
            const prefix = itemName === 'AccountA' ? 'vA' : 'vB';
    ...
    operationsToProcess.forEach(opDetails => {
      ...
      if (op.type === 'begin') {
        ...
        if (op.type === 'read') {
          ...
          const newVersionId = generateVersionId(writeOp.target === 'AccountA' ? 'vA' : 'vB');
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6 font-sans">
      <header className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold mb-1">{nonRepeatableReadScenario.name}</h1>
        <p className="text-sm text-gray-600 dark:text-gray-400">{nonRepeatableReadScenario.description}</p>
      </header>
                ...
                {versions.map(v => {
                  ...
                  return (
                    <div key={v.id} className={`p-1.5 rounded text-xs font-mono flex flex-col sm:flex-row sm:justify-between sm:items-center
                      ${isHighlighted ? 'ring-2 ring-yellow-500 dark:ring-yellow-400 bg-gray-100 dark:bg-gray-700' : 'bg-gray-50 dark:bg-gray-850'}`}>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl max-h-80 overflow-y-auto">
            ...
            {Object.values(transactionDetails).length > 0 ? Object.values(transactionDetails).map(tx => {
              ...
              const readsByItem: { [item: string]: ReadLogEntry[] } = {};
              tx.reads.forEach(read => {
                if (!readsByItem[read.item]) readsByItem[read.item] = [];
                readsByItem[read.item].push(read);
              });
              
              const hasNonRepeatableRead = Object.entries(readsByItem).some(([item, reads]) => {
                if (reads.length < 2) return false;
                return reads.some((r1, i) => reads.slice(i + 1).some(r2 => r1.value !== r2.value));
              });
              ...
              return (
                <div key={tx.id} className={`mb-2 p-2 rounded text-[11px] sm:text-xs
                  ${keyMomentInfo.highlight?.transactionState === tx.name ? 'bg-gray-100 dark:bg-gray-700 ring-1 ring-sky-500 dark:ring-sky-400' : 
                    hasNonRepeatableRead && keyMomentInfo.highlight?.nonRepeatableRead?.txName === tx.name ? 'bg-orange-50 dark:bg-orange-900/20 ring-1 ring-orange-500' :
                  ...
                  {tx.reads.map((read, idx) => {
                    const isNonRepeatableHighlight = 
                      keyMomentInfo.highlight?.nonRepeatableRead?.txName === tx.name &&
                      keyMomentInfo.highlight?.nonRepeatableRead?.item === read.item;
                    
                    // Check if this read is different from previous reads of same item
                    const previousReads = tx.reads.slice(0, idx).filter(r => r.item === read.item);
                    const isDifferentValue = previousReads.some(pr => pr.value !== read.value);
                    ...
                    return (
                      <p key={idx} className={`pl-1
                        ${keyMomentInfo.highlight?.transactionSpecificRead?.txName === tx.name &&
                          ...
                          : isNonRepeatableHighlight && isDifferentValue ? 'text-orange-600 dark:text-orange-400 font-semibold' :
                          'text-gray-700 dark:text-gray-300'}`}>
                        ↳ Read <span className="">{read.item}</span> (t={read.time}): Val <span className={isDifferentValue ? 'font-bold' : ''}>{read.value === undefined ? 'N/A' : read.value}</span> (from {read.versionId || 'N/A'})
                        {isDifferentValue && <span className="text-orange-600 dark:text-orange-400"> ⚠️ CHANGED!</span>}
                      </p>
                    );
                  })}
                </div>
...
export default ReadCommittedNonRepeatableReadVisualizer;