...
interface Version {
  ...
  category?: string; // For phantom read scenario
}
...
interface OperationBase {
  time: number;
  type: 'begin' | 'read' | 'write' | 'commit' | 'abort' | 'query';
  comment?: string;
}
...
type TransactionOperation = OperationBase | ReadOperation | WriteOperation | QueryOperation;
...
interface ScenarioKeyMomentHighlight {
  ...
  transactionSpecificQuery?: { txName: string; step: number };
  ...
  phantomRead?: { txName: string; query: string }; // Special highlight for phantom read
}
...
type CompletedTimelineOperation = OperationBase & {
  ...
  query?: string; // Present on query operations
  category?: string; // Present on query operations
...
const phantomReadScenario: ReadCommittedScenarioConfig = {
  name: "Read Committed: Phantom Read Anomaly",
  description: "Demonstrates how Read Committed isolation allows range queries to return different sets of rows (phantom rows) when other transactions insert new rows that match the query criteria.",
  initialDataItems: {
    Product1: [{ id: 'vP1', value: 100, txMin: 0, txMax: null, txMinName: 'Initial', txMaxName: null, category: 'Electronics' }],
    Product2: [{ id: 'vP2', value: 50, txMin: 0, txMax: null, txMinName: 'Initial', txMaxName: null, category: 'Books' }]
  },
  transactions: {
    T1: {
      color: '#3b82f6', // Blue-500
      operations: [
        { type: 'begin', time: 0, comment: "Add new electronics product" },
        { type: 'write', time: 10, target: 'Product3', value: 200, category: 'Electronics', comment: "New Electronics item" },
        { type: 'commit', time: 20 }
      ]
    },
    T2: {
      ...
      operations: [
        { type: 'begin', time: 5, comment: "Count electronics products" },
        { type: 'query', time: 8, query: "COUNT(*) WHERE Category='Electronics'", category: 'Electronics', comment: "First count" },
        { type: 'query', time: 25, query: "COUNT(*) WHERE Category='Electronics'", category: 'Electronics', comment: "Second count - same query!" },
        { type: 'commit', time: 30 }
      ]
    },
    T3: {
      ...
      operations: [
        { type: 'begin', time: 15, comment: "Add another electronics product" },
        { type: 'write', time: 18, target: 'Product4', value: 150, category: 'Electronics', comment: "Another Electronics item" },
        { type: 'commit', time: 23 }
      ]
    }
  },
  keyMoments: [
    { step: 2, text: "T2 executes query: COUNT(*) WHERE Category='Electronics'. Finds 1 product (Product1).", autoPause: true, highlight: { currentOpPanel: true, transactionSpecificQuery: { txName: 'T2', step: 2 } } },
    { step: 4, text: "T1 commits, adding Product3 (Electronics, $200). This product now exists in the committed state.", autoPause: true, highlight: { currentOpPanel: true, committedTx: 1, dataItemVersions: { Product3: ['vP3'] } } },
    { step: 6, text: "T3 commits, adding Product4 (Electronics, $150). Another electronics product now exists.", autoPause: true, highlight: { currentOpPanel: true, committedTx: 3, dataItemVersions: { Product4: ['vP4'] } } },
    { step: 7, text: "PHANTOM READ! T2 executes the SAME query: COUNT(*) WHERE Category='Electronics'. Now finds 3 products! Two phantom rows (Product3, Product4) appeared since the first query!", autoPause: true, isCritical: true, highlight: { currentOpPanel: true, transactionSpecificQuery: { txName: 'T2', step: 7 }, phantomRead: { txName: 'T2', query: 'Electronics' }, dataItemVersions: { Product3: ['vP3'], Product4: ['vP4'] } } }
  ]
};


// --- Read Committed Phantom Read Scenario ---
let versionCounter = 0;
const generateVersionId = (prefix: string): string => `${prefix}${versionCounter++}`;


const ReadCommittedPhantomReadVisualizer: React.FC = () => {
  ...
  const { transactions, keyMoments, initialDataItems: scenarioInitialDataItems } = phantomReadScenario;
  ...
  // Execute range query for phantom read scenario
  const executeQuery = useCallback((
    category: string,
    globalCommittedSet: Set<number>,
    allVersions: DataItemHistory
  ): { count: number; items: string[] } => {
    const matchingItems: string[] = [];
    
    Object.entries(allVersions).forEach(([itemName, versions]) => {
      const visibleVersion = findVisibleVersion(versions, globalCommittedSet);
      if (visibleVersion && visibleVersion.category === category) {
        matchingItems.push(itemName);
      }
    });
    
    return { count: matchingItems.length, items: matchingItems };
  }, [findVisibleVersion]);
  ...
  const calculateStateUpToStep = useCallback((step: number): {
    ...
    operationsToProcess.forEach(opDetails => {
      ...
      if (op.type === 'begin') {
        ...
        const queryOp = op as QueryOperation;
        ...
        if (op.type === 'read') {
          ...
          const queryResult = executeQuery(queryOp.category, tempCommittedIds, tempVersions);
          tx.queries.push({
            query: queryOp.query,
            resultCount: queryResult.count,
            matchingItems: queryResult.items,
            time: op.time
          });
        } else if (op.type === 'write') {
          const itemHistory = tempVersions[writeOp.target] || [];
          tempVersions[writeOp.target] = itemHistory;

          const newVersionId = generateVersionId('vP');
          const newVersion: Version = {
            ...
            txMinName: tx.name,
            txMaxName: null,
            category: writeOp.category
          };

          itemHistory.push(newVersion);
          tx.writesMade.push({ item: writeOp.target, newVersionId: newVersion.id });
  ...
  }, [allOperations, findVisibleVersion, executeQuery, scenarioInitialDataItems]);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6 font-sans">
      <header className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold mb-1">{phantomReadScenario.name}</h1>
        <p className="text-sm text-gray-600 dark:text-gray-400">{phantomReadScenario.description}</p>
      </header>
            ...
            <div key={itemName} className="mb-4">
              ...
              <div className="space-y-1.5 max-h-60 overflow-y-auto pr-2">
                {versions.map(v => {
                  ...
                  const isPhantom = keyMomentInfo.highlight?.phantomRead && (itemName === 'Product3' || itemName === 'Product4');
                  ...
                  return (
                    <div key={v.id} className={`p-1.5 rounded text-xs font-mono flex flex-col sm:flex-row sm:justify-between sm:items-center
                      ${isHighlighted && isPhantom ? 'ring-2 ring-purple-500 dark:ring-purple-400 bg-purple-50 dark:bg-purple-900/20' :
                      ...
                      <div className="flex-grow text-gray-700 dark:text-gray-300">
                        ...
                        Val: <span className="text-green-600 dark:text-green-400 font-semibold">${v.value}</span>,
                        Cat: <span className="text-blue-600 dark:text-blue-400">{v.category || 'N/A'}</span><br className="sm:hidden"/>
                      ...
                      <span className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 sm:mt-0 sm:ml-2 self-start sm:self-center">
                        ({versionStatus})
                        {isPhantom && <span className="text-purple-600 dark:text-purple-400 font-bold"> - PHANTOM!</span>}
                      </span>
                    </div>
                  );
                })}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className={`bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl ${currentOpIsHighlighted ? 'ring-2 ring-purple-500 dark:ring-purple-400' : ''}`}>
            ...
            {currentOpDetails ? (
              <div className={`font-mono text-xs ${currentOpIsHighlighted ? 'text-purple-600 dark:text-purple-300 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                ...
                {(currentOpDetails as WriteOperation).category && ` (${(currentOpDetails as WriteOperation).category})`}
                {(currentOpDetails as QueryOperation).query && ` "${(currentOpDetails as QueryOperation).query}"`}
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-xl max-h-80 overflow-y-auto">
            ...
            {Object.values(transactionDetails).length > 0 ? Object.values(transactionDetails).map(tx => {
              ...
              const hasPhantomRead = tx.queries.length >= 2 && 
                tx.queries.some((q1, i) => 
                  tx.queries.slice(i + 1).some(q2 => 
                    q1.query === q2.query && q1.resultCount !== q2.resultCount
                  )
                );

              return (
                <div key={tx.id} className={`mb-2 p-2 rounded text-[11px] sm:text-xs
                  ${keyMomentInfo.highlight?.transactionState === tx.name ? 'bg-gray-100 dark:bg-gray-700 ring-1 ring-sky-500 dark:ring-sky-400' : 
                    hasPhantomRead && keyMomentInfo.highlight?.phantomRead?.txName === tx.name ? 'bg-purple-50 dark:bg-purple-900/20 ring-1 ring-purple-500' :
                    'bg-gray-50 dark:bg-gray-850'}`}>
                  ...
                  {tx.queries.map((query, idx) => {
                    const isPhantomHighlight = 
                      keyMomentInfo.highlight?.phantomRead?.txName === tx.name &&
                      keyMomentInfo.highlight?.transactionSpecificQuery?.step === currentStep;
                    
                    // Check if this query result is different from previous same queries
                    const previousQueries = tx.queries.slice(0, idx).filter(q => q.query === query.query);
                    const isDifferentCount = previousQueries.some(pq => pq.resultCount !== query.resultCount);

                    return (
                      <p key={idx} className={`pl-1
                        ${keyMomentInfo.highlight?.transactionSpecificQuery?.txName === tx.name &&
                          keyMomentInfo.highlight?.transactionSpecificQuery?.step === currentStep && idx === tx.queries.length - 1
                          ? 'text-yellow-600 dark:text-yellow-300 font-semibold'
                          : isPhantomHighlight && isDifferentCount ? 'text-purple-600 dark:text-purple-400 font-semibold' :
                          'text-gray-700 dark:text-gray-300'}`}>
                        ↳ Query (t={query.time}): <span className="text-[10px]">{query.query}</span><br/>
                        <span className="pl-2">Result: {query.resultCount} rows [{query.matchingItems.join(', ')}]</span>
                        {isDifferentCount && <span className="text-purple-600 dark:text-purple-400"> 👻 PHANTOM!</span>}
                      </p>
                    );
                  })}
                  {tx.reads.map((read, idx) => (
                    <p key={`read-${idx}`} className="pl-1 text-gray-700 dark:text-gray-300">
                      ↳ Read <span className="">{read.item}</span> (t={read.time}): Val <span className="">{read.value === undefined ? 'N/A' : read.value}</span>
                    </p>
                  ))}
                </div>
              );
        ...
        <div className="space-y-3 relative" ref={transactionSectionRef}>
            ...
            <div key={txName} className="relative">
              ...
              <div className="relative h-6 bg-gray-100 dark:bg-gray-700 rounded">
                ...
                {tx.operations.map((op, idx) => {
                  ...
                  if ((op as QueryOperation).query) title = `QUERY: ${(op as QueryOperation).query}`;

                  if (opGlobalInfo && op.type === 'query') {
                    const txDetail = transactionDetails[txName];
                    const queryLogEntry = txDetail?.queries.find(q => q.time === op.time);
                    if (queryLogEntry) title += ` -> ${queryLogEntry.resultCount} rows`;
                  }
                  
                  const opChar = op.type === 'query' ? 'Q' : op.type.charAt(0).toUpperCase();
         ...
         <div className="mt-6 pt-3 border-t border-gray-200 dark:border-gray-700 text-xs">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 text-gray-600 dark:text-gray-400">
            {['Begin', 'Read', 'Write', 'Query', 'Commit', 'Abort'].map(item => (
                 <div key={item} className="flex items-center gap-1.5">
                    <div className="w-5 h-5 rounded-full border-2 border-gray-400 dark:border-gray-500 bg-gray-50 dark:bg-gray-700 flex items-center justify-center font-bold text-gray-700 dark:text-gray-200">
                      {item === 'Query' ? 'Q' : item.charAt(0)}
                    </div>
                    <span>{item}</span>
                 </div> ))}
          </div>
...
export default ReadCommittedPhantomReadVisualizer;