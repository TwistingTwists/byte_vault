...
const MvccVisibilityVisualizer: React.FC = () => {
  // ... (All existing useState, useRef, useEffect, useCallback, useMemo hooks are the same)
  ...
  const timelineRef = useRef<HTMLDivElement>(null); // Ref for the timeline container
  const transactionSectionRef = useRef<HTMLDivElement>(null); // Ref for the section containing transaction bars
 ...
 const updateStateForStep = useCallback((newStep: number) => {
  ...
  // Calculate the height for the time marker dynamically or use a sufficiently large fixed one
  const [timelineMarkerHeight, setTimelineMarkerHeight] = useState('10rem'); // Default/fallback

  useEffect(() => {
    if (transactionSectionRef.current) {
      // Get the height of the transaction bars section and add some padding
      // This is a bit of a hack; ideally, the marker would be positioned relative to this section.
      // For now, we set a height that should be enough.
      // A more robust way would be to absolutely position the marker relative to the timeline axis (top-4)
      // and set its bottom to align with the bottom of the transactionSectionRef.
      // Or, calculate based on number of transactions * (bar_height + space_y).
      const numTransactions = Object.keys(transactions).length;
      const barHeight = 2; // h-8 (2rem) but using rem based on font-size is tricky here. Let's use a pixel-based estimation.
                          // h-6 is 1.5rem. Assuming 1rem = 16px, 1.5rem = 24px.
      const spaceBetween = 1; // space-y-4 -> 1rem = 16px
      // Rough estimate: numTransactions * (24px + 16px) - 16px (no space after last)
      // For 3 transactions: 3 * (24+16) - 16 = 3 * 40 - 16 = 120 - 16 = 104px ~= 6.5rem
      // Let's make it a bit more generous to cover headers and padding.
      // A fixed value might be simpler if the layout is static.
      setTimelineMarkerHeight(`${numTransactions * (1.5 + 1) - 1 + 1}rem`); // Approx numTx * (h-6 + space-y-4) + small_padding
                                                                        // (1.5rem for h-6, 1rem for space-y-4)
    }
  }, [transactions]);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6 font-sans">
      {/* ... (Header and Controls section remains the same as before) ... */}
      ...
      {/* ... (State Displays Grid section remains the same as before) ... */}
      ...
      {/* ... (Key Moment Panel section remains the same as before) ... */}
      {keyMomentInfo.text && (
      ...
      {/* Timeline Visualization - MODIFIED SECTION */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 sm:p-6" ref={timelineRef}>
        <h2 className="text-base sm:text-xl font-semibold mb-2 text-gray-800 dark:text-gray-200">Transaction Timelines</h2> {/* Reduced mb-4 to mb-2 */}
        
        {/* Time Axis and Marker Container */}
        <div className="relative mb-4 pt-4"> {/* Reduced mb-8 to mb-4 */}
          <div className="absolute top-4 left-0 right-0 h-px bg-gray-300 dark:bg-gray-600 z-0"></div> {/* Timeline axis line */}
          <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-2 px-[2.5%] w-[95%] relative z-10"> {/* Time labels */}
          ...
          {/* Current Time Marker - Spanning across transactions */}
          {(isRunning || currentStep > 0) && (
            <div 
              className="absolute top-4 w-0.5 bg-red-500 transition-all duration-200 ease-linear z-20" // top-4 aligns with axis line
              style={{ 
                left: `${getTimePosition(currentTime)}%`,
                height: timelineMarkerHeight // Calculated or fixed height to span transactions
              }}
            >
              <div className="absolute -top-3.5 -left-[7px] w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-white dark:border-gray-900 shadow-md"></div> {/* Marker head, adjusted top to sit above axis */}
            </div>
          )}
        </div>

        {/* Transaction Bars Section */}
        <div className="space-y-3 relative" ref={transactionSectionRef}> {/* Reduced space-y-6 to space-y-3, added relative for z-indexing context if needed */}
          {Object.entries(transactions).map(([txName, tx]) => (
            <div key={txName} className="relative">
              <div className="flex items-center mb-0.5"> {/* Reduced mb-1 to mb-0.5 */}
                <div className="w-2.5 h-2.5 rounded-full mr-1.5 flex-shrink-0" style={{ backgroundColor: tx.color }}></div> {/* Smaller color dot */}
                <h3 className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200">{txName}</h3>
              </div>
              <div className="relative h-6 bg-gray-100 dark:bg-gray-700 rounded"> {/* MODIFIED: h-6 for transaction bar */}
                ...
                {tx.operations.map((op, idx) => {
                  ...
                  if (opGlobalInfo && op.type === 'read') {
                    ...
                    // Find the specific read log entry by time and target for this operation instance
                  ...
                  return (
                    <div
                      ...
                      style={{ 
                        left: `calc(${getTimePosition(op.time)}% - 10px)`, /* half of w-5 */
                        borderColor: isActiveOp || isHighlightedOp ? tx.color : undefined, // Use dynamic border color only when active/highlighted
                      ...
                      {op.type.charAt(0).toUpperCase()}
                       {isActiveOp && (
                        <div className="absolute bottom-full mb-1 left-1/2 transform -translate-x-1/2 bg-black text-white px-1.5 py-0.5 rounded text-[9px] whitespace-nowrap z-30 opacity-0 group-hover:opacity-100 pointer-events-none">
                          {title}
                          <div className="absolute top-full left-1/2 transform -translate-x-1/2 border-[3px] border-transparent border-t-black"></div> {/* Smaller arrow */}
                        </div>
                      )}
                    </div>
                  );
        ...
        {/* ... (Legend section remains the same as before) ... */}
